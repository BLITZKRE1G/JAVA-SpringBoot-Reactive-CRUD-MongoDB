...
public class EmployeeService {
    ...
    public Flux<Employee> getByEmploymentStatus(String employmentStatus){
        return repository.findByStatus(employmentStatus);
    }

    public Mono<PaginatedEmployee> getEmployeesByPagination(EmployeeFilter employeeFilter, String _id){
        if(_id == null){
        ...
        if(employeeFilter.getSortValue() != null && employeeFilter.getSortOrder() != null){
            page = PageRequest.of(employeeFilter.getPage() - 1, employeeFilter.getCount(), Sort.by(getSortingOrders(employeeFilter.getSortValue(), employeeFilter.getSortOrder())));
        }
